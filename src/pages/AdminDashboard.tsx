...
const getApiBaseUrl = () => {
  ...
  // Development: Use relative paths to leverage Vite dev proxy
...
const AdminDashboard = () => {
  ...
  const [currentUserVideoClips, setCurrentUserVideoClips] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const recordingsInterval = setInterval(() => {
      if (currentUserPhotos.length === 0 && currentUserVideoClips.length === 0 && currentUserAudioClips.length === 0) {
        loadRecordings();
      }
    }, 3000);
  ...
  }, [currentUserPhotos, currentUserVideoClips, currentUserAudioClips]);
  ...
  const loadRecordings = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (data.photos !== undefined || data.videoClips !== undefined || data.audioClips !== undefined) {
          // Single user data with photos, videos, and audio clips
          setCurrentUserPhotos(data.photos || []);
          setCurrentUserVideoClips(data.videoClips || []);
          setCurrentUserAudioClips(data.audioClips || []);
          setAllUsers([]);
          console.log(`✅ Loaded ${data.photos?.length || 0} photos, ${data.videoClips?.length || 0} videos, and ${data.audioClips?.length || 0} audio clips for ${data.userName}`);
  ...
  const deleteAllRecordings = async (type: "frame" | "video" | "audio") => {
    try {
      let itemsToDelete: any[] = [];
      if (type === "frame") {
        itemsToDelete = currentUserPhotos;
      } else if (type === "video") {
        itemsToDelete = currentUserVideoClips;
      } else {
        itemsToDelete = currentUserAudioClips;
      }
      
      if (itemsToDelete.length === 0) {
        toast({
          title: "No Files",
          description: `No ${type === "frame" ? "photos" : type === "video" ? "videos" : "audio"} to delete.`,
        });
        return;
      }
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          type: type === "frame" ? "frame" : type === "video" ? "video" : "audio"
        })
      }).catch(err => {
      ...
      toast({
        title: "All Deleted",
        description: `${itemsToDelete.length} ${type === "frame" ? "photo(s)" : type === "video" ? "video(s)" : "audio file(s)"} deleted successfully.`,
      });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className={`mt-1 ${theme === 'dark' ? 'text-gray-400' : ''}`}>
                    {selectedAlert ? `📸 Video frames, 🎬 videos and 🔊 audio clips from ${studentProfile?.full_name || 'Student'}` : (currentUserPhotos.length > 0 || currentUserVideoClips.length > 0 || currentUserAudioClips.length > 0) ? `📊 Viewing ${currentUserPhotos.length} photos, ${currentUserVideoClips.length} videos and ${currentUserAudioClips.length} audio clips` : '👥 Recordings organized by student'}
                  </CardDescription>
                </div>
              </div>
              <div className="flex gap-2">
                {(currentUserPhotos.length > 0 || currentUserVideoClips.length > 0 || currentUserAudioClips.length > 0) ? (
                  <>
                    <button
                      onClick={() => {
                        loadRecordings();
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-semibold flex items-center gap-2">
                        ...
                        Videos ({currentUserVideoClips.length})
                      </h3>
                      {currentUserVideoClips.length > 0 && (
                        <button
                          onClick={() => deleteAllRecordings("video")}
                          className="px-3 py-1 text-xs bg-destructive text-destructive-foreground rounded hover:bg-destructive/90"
                        >
                          <Trash2 className="h-3 w-3 inline mr-1" />
                          Delete All Videos
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {currentUserVideoClips.length > 0 ? (
                        currentUserVideoClips
                          .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
                          .map((videoClip) => {
                            const apiBaseUrl = getApiBaseUrl();
                            // Use the backend API endpoint to fetch video by ID
                            const videoUrl = `${apiBaseUrl}/api/video/${videoClip.id}`;
                            return (
                              <div key={videoClip.id} className="border rounded-lg p-4 bg-muted/30 hover:bg-muted/50 transition-colors">
                                <div className="flex items-start justify-between mb-3">
                                  <Badge variant="secondary">Video</Badge>
                                  <span className="text-xs text-muted-foreground">{(videoClip.size / 1024 / 1024).toFixed(1)} MB</span>
                                </div>
                                <video 
                                  controls 
                                  className="w-full mb-2 bg-black rounded"
                                  controlsList="nodownload"
                                  preload="metadata"
                                  onError={(e) => {
                                    console.error(`❌ Video playback error for ${videoClip.id}:`, e);
                                  }}
                                >
                                  <source 
                                    src={videoUrl} 
                                    type={videoClip.mimeType || "video/webm"}
                                  />
                                  <p>Your browser does not support the video element. <a href={videoUrl} download>Download video</a></p>
                                </video>
                                <div className="text-xs text-muted-foreground mb-2">
                                  {new Date(videoClip.timestamp).toLocaleTimeString()}
                                </div>
                                <div className="flex gap-2">
                                  <a 
                                    href={videoUrl}
                                  ...
                                  <button
                                    onClick={() => setDeleteConfirmFile(videoClip)}
                        ...
                        <p className="col-span-full text-center text-sm text-muted-foreground py-4">No videos yet</p>
                      )}
                    </div>
                  </div>
              ...
              <div className="text-center py-12 space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Recordings will appear here once students trigger an emergency alert and the system starts capturing video frames, videos, and audio clips.
                  </p>
                </div>
